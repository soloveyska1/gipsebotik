...
# 2. ФАЙЛ database/core.py
core_content = """import sqlite3
import logging
...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, balance INTEGER DEFAULT 0, total_spent INTEGER DEFAULT 0, orders_count INTEGER DEFAULT 0, is_banned INTEGER DEFAULT 0, referrer_id INTEGER DEFAULT 0, joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        cursor.execute("CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, service_type TEXT, topic TEXT, deadline TEXT, status TEXT DEFAULT 'checking', price INTEGER DEFAULT 0, files TEXT, speech INTEGER DEFAULT 0, pres INTEGER DEFAULT 0, vip INTEGER DEFAULT 0, is_visible INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, deadline_type TEXT, upsell INTEGER DEFAULT 0)")
        cursor.execute("CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, order_id INTEGER, sender_id INTEGER, is_admin INTEGER, msg_type TEXT, content TEXT, file_id TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        cursor.execute("CREATE TABLE IF NOT EXISTS transactions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount INTEGER, reason TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        cursor.execute("CREATE TABLE IF NOT EXISTS reviews (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, text TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
...
async def create_order(data):
    conn = await get_connection()
    try:
        cursor = conn.execute("INSERT INTO orders (user_id, service_type, topic, deadline_type, price, files, speech, pres, vip, status, deadline) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, 'checking', ?)", 
...
async def get_user(user_id):
    ...
    try:
        ...
        row = cursor.fetchone()
        if row:
            return {"user_id": row[0], "username": row[1], "full_name": row[2], "balance": row[3], "total_spent": row[4], "orders_count": row[5], "is_banned": row[6], "referrer_id": row[7], "joined_at": row[8]}
        return None
    finally:
        conn.close()
...
async def get_order(order_id):
    ...
    try:
        ...
        row = cursor.fetchone()
        if row:
            return {"id": row[0], "user_id": row[1], "service_type": row[2], "topic": row[3], "deadline": row[4], "status": row[5], "price": row[6], "files": row[7], "created_at": row[12], "deadline_type": row[13]}
        return None
    finally:
        conn.close()
...
async def add_chat_message(order_id, sender_id, is_admin, msg_type, content, file_id=None):
    conn = await get_connection()
    try:
        conn.execute("INSERT INTO messages (order_id, sender_id, is_admin, msg_type, content, file_id) VALUES (?, ?, ?, ?, ?, ?)", (order_id, sender_id, 1 if is_admin else 0, msg_type, content, file_id))
        conn.commit()
    finally:
        conn.close()
...
with open("database/core.py", "w", encoding="utf-8") as f:
    f.write(core_content)

# 3. ФАЙЛ database/__init__.py
with open("database/__init__.py", "w") as f:
    f.write("")

# 4. ФАЙЛ handlers/__init__.py
with open("handlers/__init__.py", "w") as f:
    f.write("")

print("✅ FIX COMPLETED: core.py is restored. init_db() is there.")
